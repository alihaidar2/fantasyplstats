...
async function updateData() {
  const client = await pool.connect();
    try {
      ...
      await updateTeams(client);
      await updateFixtures(client);
  ...
  async function deleteAllData(client) {
    ...
    await client.query(`DELETE FROM GAMEWEEKS;`);
    console.log("Deleted data")
  }
  
  
  // Insert the Season
  async function updateSeasons(client) {
    const sql = `
      INSERT INTO Seasons (
  ...
  async function updatePlayers(client) {
  ...
  async function updateFixtures(client) {
    ...
    for (const fixture of fixtures) {
      if (fixture.event != null) {
        await insertFixture(fixture, client);
      }
    }
    console.log("All fixtures have been inserted successfully.");
  }
  
  async function insertFixture(fixture, client) {
    const sql = `
  ...
  async function updateTeams(client) {
    const url = "https://fantasy.premierleague.com/api/bootstrap-static/";
    ...
    for (const team of teams) {
      await insertTeam(team, client);
    }
    console.log("All teams have been inserted successfully.");
  }
  
  // Insert a team into the database - used by updateTeams()
  async function insertTeam(team, client) {
    const sql = `
  ...
  async function updateGameweeks(client) {
    ...
    for (const gameweek of gameweeks) {
      await insertGameweek(gameweek, client);
    }
    console.log("All gameweeks have been inserted successfully.");
  }
  
  async function insertGameweek(gameweek, client) {
    const sql = `
...
module.exports = {initializeDatabase, updateData};
